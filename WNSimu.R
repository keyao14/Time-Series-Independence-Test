    ...
    Err <- foreach(rep=1:1000, .combine='cbind',.packages = depend_packages) %dopar%
      {
        ...
        pvalue=c(wntest(X,w,m,5,500),wntest_kendall(X,w,m,5,500),wntest_dcov(X,w,m,5,500),Box.test(X,lag=5,type="Ljung-Box")[["p.value"]],Box.test(DX,lag=5,type="Ljung-Box")[["p.value"]])
        pvalue
      }
